...
$(AS_BINARIES_PATH)/$(AS_CONFIGURATION)/$(AS_PLC)/Transfer.lst: \
	...
	$(AS_PROJECT_PATH)/Physical/Simulation/PLC1/SimulationOnly.sw \
	$(AS_CPU_PATH)/WebServ.br \
	$(AS_CPU_PATH)/ArSdm.br \
	$(AS_CPU_PATH)/ArSdmSvg.br \
	$(AS_CPU_PATH)/ArSdmSvp.br \
	$(AS_CPU_PATH)/ArSdmHtm.br
	@"$(AS_BIN_PATH)/BR.AS.FinalizeBuild.exe" "$(AS_PROJECT_PATH)/bms.apj" -t "$(AS_TEMP_PATH)" -o "$(AS_BINARIES_PATH)" -c "$(AS_CONFIGURATION)" -i "C:/Program Files/BrAutomation/AS30080" -S PLC1  -pil   -C "/RT=1000  /DAIP=127.0.0.1 /CKDA=0 /REPO=11160 /ANSL=1" -D "/IF=tcpip /LOPO=11159 /SA=1"

#nothing to do (just call module make files)

include $(AS_CPU_PATH)/iomap/iomap.mak
include $(AS_CPU_PATH)/hs_sim/hs_sim.mak
